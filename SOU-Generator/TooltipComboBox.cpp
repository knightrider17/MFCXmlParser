...
// Nice work!  The post of (2001/04/24) works fine for fixing  
// the SelChange notification, why does the question keep being asked?
// I added some code to help with my SORTED combo box because when 
// I was using AddString my string indexes were changing but my tooltip indices were not. 
// It looked like this:


//---------------------------------------------------
//*
int CTooltipComboBox::AddStringAndTooltip( const CString &dropDownString, const CString &toolTip)
{
...
void CTooltipComboBox::PreSubclassWindow() 
{
	...
	DWORD dwStyle =  WS_POPUP | WS_BORDER |
					 LVS_REPORT | LVS_NOCOLUMNHEADER | LVS_SINGLESEL | LVS_OWNERDATA;
...
// ”правление длинной выпадающего списка
	SetDroppedHeight( rcAll.Height( ) + rc.Height( ) );
// Ѕыло и показывало только один элемент списка
	//SetDroppedHeight( rcAll.Height( ) - rc.Height( ) );
